...
from mamba.agent.utils.params import FreezeParameters
from mamba.networks.dreamer.rnns import rollout_representation, rollout_policy
...
def model_loss(config, model, obs, action, av_action, reward, done, fake, last):
    ...
    prev_state = model.representation.initial_state(
        batch_size, n_agents, device=obs.device
    )
    prior, post, deters = rollout_representation(
        model.representation, time_steps, embed, action, prev_state, last
    )
    ...
    av_action_loss = (
        log_prob_loss(model.av_action, feat_dec, av_action[:-1])
    ...
    model_loss = (
        div + reward_loss + dis_loss + reconstruction_loss + pcont_loss + av_action_loss
    )
...
def actor_rollout(obs, action, last, model, actor, critic, config):
    ...
    with FreezeParameters([model]):
        ...
        prev_state = model.representation.initial_state(
            obs.shape[1], obs.shape[2], device=obs.device
        )
        prior, post, _ = rollout_representation(
            model.representation, obs.shape[0], embed, action, prev_state, last
        )
        post = post.map(
            lambda x: x.reshape((obs.shape[0] - 1) * obs.shape[1], n_agents, -1)
        )
        items = rollout_policy(
            model.transition, model.av_action, config.HORIZON, actor, post
        )
    imag_feat = items["imag_states"].get_features()
    imag_rew_feat = torch.cat(
        [items["imag_states"].stoch[:-1], items["imag_states"].deter[1:]], -1
    )
...
def critic_rollout(model, critic, states, rew_states, actions, raw_states, config):
    with FreezeParameters([model, critic]):
        ...
        imag_reward = (
            imag_reward.reshape(actions.shape[:-1])
            .unsqueeze(-1)
            .mean(-2, keepdim=True)[:-1]
        )
        value = critic(states)
        discount_arr = model.pcont(rew_states).mean
        wandb.log(
            {
                "Critic/Max reward": imag_reward.max(),
                "Critic/Min reward": imag_reward.min(),
                "Critic/Reward": imag_reward.mean(),
                "Critic/Discount": discount_arr.mean(),
...
def actor_loss(
    imag_states, actions, av_actions, old_policy, advantage, actor, ent_weight
):
    ...
    rho = (
        F.log_softmax(new_policy, dim=-1).gather(2, actions)
